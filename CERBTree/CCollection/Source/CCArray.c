...
CF_INLINE uint32_t __CFArrayDequeRoundUpCapacity(uint32_t capacity) {
...
CF_INLINE uint32_t __CFArrayGetType(CFArrayRef array) {
    return __CFRuntimeGetValue(array, 1, 0);
}

CF_INLINE uint32_t __CFArrayGetSizeOfType(uint32_t t) {
    uint32_t size = 0;
...
CF_INLINE uint32_t __CFArrayGetCount(CFArrayRef array) {
    return array->_count;
}

CF_INLINE void __CFArraySetCount(CFArrayRef array, uint32_t v) {
...
CF_INLINE struct __CFArrayBucket *__CFArrayGetBucketAtIndex(CFArrayRef array, uint32_t idx) {
    switch (__CFArrayGetType(array)) {
        case __kCFArrayImmutable:
        case __kCFArrayDeque:
...
static CFArrayRef __CFArrayInit(CFAllocatorRef allocator, UInt32 flags, uint32_t capacity, const CFArrayCallBacks *callBacks) {
...
CF_PRIVATE CFArrayRef __CFArrayCreateTransfer(CFAllocatorRef allocator, const void **values, uint32_t numValues) {
...
CF_PRIVATE CFArrayRef __CFArrayCreate0(CFAllocatorRef allocator, const void **values, uint32_t numValues, const CFArrayCallBacks *callBacks) {
    CFArrayRef result;
    const CFArrayCallBacks *cb;
...
CF_PRIVATE CFMutableArrayRef __CFArrayCreateMutable0(CFAllocatorRef allocator, uint32_t capacity, const CFArrayCallBacks *callBacks) {
...
CF_PRIVATE CFMutableArrayRef __CFArrayCreateMutableCopy0(CFAllocatorRef allocator, uint32_t capacity, CFArrayRef array) {
...
CFArrayRef CFArrayCreate(CFAllocatorRef allocator, const void **values, uint32_t numValues, const CFArrayCallBacks *callBacks) {
...
CFMutableArrayRef CFArrayCreateMutableCopy(CFAllocatorRef allocator, uint32_t capacity, CFArrayRef array) {
    return __CFArrayCreateMutableCopy0(allocator, capacity, array);
}

#endif

CF_PRIVATE uint32_t _CFNonObjCArrayGetCount(CFArrayRef array) {
    __CFGenericValidateType(array, CFArrayGetTypeID());
    CHECK_FOR_MUTATION(array);
    return __CFArrayGetCount(array);
}

uint32_t CFArrayGetCount(CFArrayRef array) {
...
uint32_t CFArrayGetCountOfValue(CFArrayRef array, CFRange range, const void *value) {
...
const void *CFArrayGetValueAtIndex(CFArrayRef array, uint32_t idx) {
...
const void *_CFArrayCheckAndGetValueAtIndex(CFArrayRef array, uint32_t idx, Boolean *outOfBounds) {
    CHECK_FOR_MUTATION(array);
...
uint32_t CFArrayGetFirstIndexOfValue(CFArrayRef array, CFRange range, const void *value) {
    uint32_t idx;
    __CFGenericValidateType(array, CFArrayGetTypeID());
    __CFArrayValidateRange(array, range, __PRETTY_FUNCTION__);
    CHECK_FOR_MUTATION(array);
...
uint32_t CFArrayGetLastIndexOfValue(CFArrayRef array, CFRange range, const void *value) {
    uint32_t idx;
    __CFGenericValidateType(array, CFArrayGetTypeID());
    __CFArrayValidateRange(array, range, __PRETTY_FUNCTION__);
    CHECK_FOR_MUTATION(array);
...
static void __CFArrayRepositionDequeRegions(CFMutableArrayRef array, CFRange range, uint32_t newCount) {
    ...
    if (deque->_capacity < (uint32_t)futureCnt || (cnt < futureCnt && L + R < wiggle)) {
        ...
        uint32_t capacity = __CFArrayDequeRoundUpCapacity(futureCnt + wiggle);
        uint32_t size = sizeof(struct __CFArrayDeque) + capacity * sizeof(struct __CFArrayBucket);
        CFAllocatorRef allocator = __CFGetAllocator(array);
...
static void __CFArrayHandleOutOfMemory(CFTypeRef obj, uint32_t numBytes) {
...
void _CFArraySetCapacity(CFMutableArrayRef array, uint32_t cap) {
    ...
    if (__CFArrayGetType(array) == __kCFArrayDeque) {
        struct __CFArrayDeque *deque = (struct __CFArrayDeque *)array->_store;
        uint32_t capacity = __CFArrayDequeRoundUpCapacity(cap);
        uint32_t size = sizeof(struct __CFArrayDeque) + capacity * sizeof(struct __CFArrayBucket);
        CFAllocatorRef allocator = __CFGetAllocator(array);
        if (NULL == deque) {
...
void _CFArrayReplaceValues(CFMutableArrayRef array, CFRange range, const void **newValues, uint32_t newCount) {
    ...
    if (0) {
        ...
        if (0) {
            ...
            uint32_t capacity = __CFArrayDequeRoundUpCapacity(futureCnt);
            uint32_t size = sizeof(struct __CFArrayDeque) + capacity * sizeof(struct __CFArrayBucket);
            deque = (struct __CFArrayDeque *)CFAllocatorAllocate((allocator), size, 0);
            if (__CFOASafe) __CFSetLastAllocationEventName(deque, "CFArray (store-deque)");
...
uint32_t CFArrayBSearchValues(CFArrayRef array, CFRange range, const void *value, CFComparatorFunction comparator, void *context) {
    FAULT_CALLBACK((void **)&(comparator));
    __CFArrayValidateRange(array, range, __PRETTY_FUNCTION__);
    CFAssert1(NULL != comparator, __kCFLogAssertion, "%s(): pointer to comparator function may not be NULL", __PRETTY_FUNCTION__);
    // implemented abstractly, careful!
