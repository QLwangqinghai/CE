...
//        return -1;
//    }
//    if (timeout == fe->readTimeoutInterval) {
//        return 0;
//    }
...
//    }
//
//    if (timeout > fe->readTimeoutInterval) {//时间加长
//        uint32_t offset = timeout - fe->readTimeoutInterval;
//        CEFileEventRemoveFromReadTimer(eventLoop, fe);
//        fe->readTimeoutInterval = 0;
//        
...
//        return 0;
//    } else {//时间缩短
//        uint32_t offset = fe->readTimeoutInterval - timeout;
//        
//        CEFileEventRemoveFromReadTimer(eventLoop, fe);
//        fe->readTimeoutInterval = 0;
//        
...
CEResult_t CEFileEventSourceInit(CERunLoop_s * _Nonnull eventLoop, CEFileDescription_s fd, CEFileEventHandler_f _Nonnull handler, void * _Nullable clientData) {
    ...
    if (fd.fd >= eventLoop->setsize) {
        int result = CERunLoopResizeSetSize(eventLoop, fd.fd + 1);
        if (0 != result) {
            return result;
        }
    }
