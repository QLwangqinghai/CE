...
//            self.layout(orientation: parent.orientation, orientationSafeAreaInsets: parent.contentSafeAreaInsets, center: parent.contentCenter, contentSize: parent.contentSize)
            ...
            self.scrollView.transform = parent.contentTransform
            var bounds = self.scrollView.bounds
            bounds.size = contentSize
            self.scrollView.bounds = bounds
            self.scrollView.center = parent.contentCenter
            var paddingX: CGFloat = bounds.size.width - safeInset.left - safeInset.right - contentSize.width
            var paddingY: CGFloat = bounds.size.height - safeInset.top - safeInset.bottom - contentSize.height
            if paddingX < 0 {
                paddingX = 0
            ...
            let newContentInset = UIEdgeInsets(top: safeInset.top + paddingInset.top, left: safeInset.left + paddingInset.left, bottom: safeInset.bottom + paddingInset.bottom, right: safeInset.right + paddingInset.right)
            self.scrollView.contentInset = newContentInset
            self.scrollView.contentOffset.x = newContentInset.left * -1
            self.scrollView.contentOffset.y = newContentInset.top * -1
        }
    }
    func didMoveTo(orientationView: OrientationView?) {}
