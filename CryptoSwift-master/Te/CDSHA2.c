...
static inline void CDSHA2Process64(const uint8_t block[_Nonnull 128], uint64_t currentHash[_Nonnull 8]) {
    ...
    for (; idx<kCount; idx++) {
        uint64_t s0 = CDUInt64RotateRight(M[idx - 15], 1) ^ CDUInt64RotateRight(M[idx - 15], 8) ^ (M[idx - 15] >> 7);
        uint64_t s1 = CDUInt64RotateRight(M[idx - 2], 19) ^ CDUInt64RotateRight(M[idx - 2], 61) ^ (M[idx - 2] >> 6);
