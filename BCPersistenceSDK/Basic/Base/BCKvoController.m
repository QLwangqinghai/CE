...
static NSString *BCKeyValueObservingOptionDescription(NSKeyValueObservingOptions option)
{
  switch (option) {
...
static void BCKvoControllerAppendOptionDescription(NSMutableString *s, NSUInteger option)
{
  if (0 == s.length) {
    [s appendString:BCKeyValueObservingOptionDescription(option)];
  } else {
    [s appendString:@"|"];
    [s appendString:BCKeyValueObservingOptionDescription(option)];
  }
...
static NSUInteger BCKvoControllerEnumerateOptions(NSUInteger *ptrFlags)
{
...
static NSString *BCKeyValueObservingOptionDescriptions(NSKeyValueObservingOptions options)
{
  NSMutableString *s = [NSMutableString string];
  NSUInteger option;
  while (0 != (option = BCKvoControllerEnumerateOptions(&options))) {
    BCKvoControllerAppendOptionDescription(s, option);
  }
  return s;
}

#pragma mark _BCKvoInfo -

typedef NS_ENUM(uint8_t, _BCKvoInfoState) {
  _BCKvoInfoStateInitial = 0,

  // whether the observer registration in Foundation has completed
  _BCKvoInfoStateObserving,

  // whether `unobserve` was called before observer registration in Foundation has completed
  // this could happen when `NSKeyValueObservingOptionInitial` is one of the NSKeyValueObservingOptions
  _BCKvoInfoStateNotObserving,
};

NSString *const BCKvoNotificationKeyPathKey = @"BCKvoNotificationKeyPathKey";
 ...
 */
@interface _BCKvoInfo : NSObject
@end

@implementation _BCKvoInfo
{
@public
  __weak BCKvoController *_controller;
  NSString *_keyPath;
  NSKeyValueObservingOptions _options;
  SEL _action;
  void *_context;
  BCKvoNotificationBlock _block;
  _BCKvoInfoState _state;
}

- (instancetype)initWithController:(BCKvoController *)controller
                           keyPath:(NSString *)keyPath
                           options:(NSKeyValueObservingOptions)options
                             block:(nullable BCKvoNotificationBlock)block
...
- (instancetype)initWithController:(BCKvoController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(BCKvoNotificationBlock)block
{
  return [self initWithController:controller keyPath:keyPath options:options block:block action:NULL context:NULL];
}

- (instancetype)initWithController:(BCKvoController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options action:(SEL)action
{
  return [self initWithController:controller keyPath:keyPath options:options block:NULL action:action context:NULL];
}

- (instancetype)initWithController:(BCKvoController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context
{
  return [self initWithController:controller keyPath:keyPath options:options block:NULL action:NULL context:context];
}

- (instancetype)initWithController:(BCKvoController *)controller keyPath:(NSString *)keyPath
{
...
- (BOOL)isEqual:(id)object
{
  ...
  return [_keyPath isEqualToString:((_BCKvoInfo *)object)->_keyPath];
}
...
- (NSString *)debugDescription
{
  ...
  if (0 != _options) {
    [s appendFormat:@" options:%@", BCKeyValueObservingOptionDescriptions(_options)];
  }
  if (NULL != _action) {
...
@implementation _BCKvoSharedController
{
  NSHashTable<_BCKvoInfo *> *_infos;
  pthread_mutex_t _mutex;
}

+ (instancetype)sharedController
{
  static _BCKvoSharedController *_controller = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    _controller = [[_BCKvoSharedController alloc] init];
  });
  return _controller;
}
...
- (void)observe:(id)object info:(nullable _BCKvoInfo *)info
{
  ...
  if (info->_state == _BCKvoInfoStateInitial) {
    info->_state = _BCKvoInfoStateObserving;
  } else if (info->_state == _BCKvoInfoStateNotObserving) {
    // this could happen when `NSKeyValueObservingOptionInitial` is one of the NSKeyValueObservingOptions,
...
- (void)unobserve:(id)object info:(nullable _BCKvoInfo *)info
{
  ...
  // remove observer
  if (info->_state == _BCKvoInfoStateObserving) {
    [object removeObserver:self forKeyPath:info->_keyPath context:(void *)info];
  }
  info->_state = _BCKvoInfoStateNotObserving;
}

- (void)unobserve:(id)object infos:(nullable NSSet<_BCKvoInfo *> *)infos
{
  if (0 == infos.count) {
    return;
  }
                       ...
                       context:(nullable void *)context
{
  ...
  if (nil != info) {
    ...
    BCKvoController *controller = info->_controller;
    if (nil != controller) {
      ...
      if (nil != observer) {
        ...
        if (info->_block) {
          ...
          if (keyPath) {
            NSMutableDictionary<NSString *, id> *mChange = [NSMutableDictionary dictionaryWithObject:keyPath forKey:BCKvoNotificationKeyPathKey];
...
#pragma mark BCKvoController -

@implementation BCKvoController
{
  NSMapTable<id, NSMutableSet<_BCKvoInfo *> *> *_objectInfosMap;
...
- (NSString *)debugDescription
{
  ...
  for (id object in _objectInfosMap) {
    ...
    [infos enumerateObjectsUsingBlock:^(_BCKvoInfo *info, BOOL *stop) {
      [infoDescriptions addObject:info.debugDescription];
    }];
...
- (void)_observe:(id)object info:(_BCKvoInfo *)info
{
  // lock
  pthread_mutex_lock(&_lock);
  ...
  _BCKvoInfo *existingInfo = [infos member:info];
  if (nil != existingInfo) {
  ...
  [[_BCKvoSharedController sharedController] observe:object info:info];
}

- (void)_unobserve:(id)object info:(_BCKvoInfo *)info
{
  // lock
  pthread_mutex_lock(&_lock);
  ...
  _BCKvoInfo *registeredInfo = [infos member:info];
  ...
  // unobserve
  [[_BCKvoSharedController sharedController] unobserve:object info:registeredInfo];
}
...
- (void)_unobserve:(id)object
{
  ...
  // unobserve
  [[_BCKvoSharedController sharedController] unobserve:object infos:infos];
}
...
- (void)_unobserveAll
{
  ...
  _BCKvoSharedController *shareController = [_BCKvoSharedController sharedController];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(BCKvoNotificationBlock)block
{
  ...
  // create info
  _BCKvoInfo *info = [[_BCKvoInfo alloc] initWithController:self keyPath:keyPath options:options block:block];
...
- (void)observe:(nullable id)object keyPaths:(NSArray<NSString *> *)keyPaths options:(NSKeyValueObservingOptions)options block:(BCKvoNotificationBlock)block
{
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options action:(SEL)action
{
  ...
  // create info
  _BCKvoInfo *info = [[_BCKvoInfo alloc] initWithController:self keyPath:keyPath options:options action:action];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(nullable void *)context
{
  ...
  // create info
  _BCKvoInfo *info = [[_BCKvoInfo alloc] initWithController:self keyPath:keyPath options:options context:context];
...
- (void)unobserve:(nullable id)object keyPath:(NSString *)keyPath
{
  ...
  _BCKvoInfo *info = [[_BCKvoInfo alloc] initWithController:self keyPath:keyPath];
