...
/**
 ...
 BCKvoKeyPath(string.length) => @"length"

 Or even the complex case:

 BCKvoKeyPath(string.lowercaseString.length) => @"lowercaseString.length".
 */
#define BCKvoKeyPath(KEYPATH) \
@(((void)(NO && ((void)KEYPATH, NO)), \
({ const char *BCKvokeypath = strchr(#KEYPATH, '.'); NSCAssert(BCKvokeypath, @"Provided key path is invalid."); BCKvokeypath + 1; })))
...
/**
 ...
 BCKvoClassKeyPath(NSString, length) => @"length"
 BCKvoClassKeyPath(NSString, lowercaseString.length) => @"lowercaseString.length"
 */
#define BCKvoClassKeyPath(CLASS, KEYPATH) \
@(((void)(NO && ((void)((CLASS *)(nil)).KEYPATH, NO)), #KEYPATH))

NS_ASSUME_NONNULL_BEGIN

/**
 Key provided in the @c change dictionary of @c BCKvoNotificationBlock that's value represents the key-path being observed
 */
extern NSString *const BCKvoNotificationKeyPathKey;
...
/**
 ...
 @param change The change dictionary which also includes @c BCKvoNotificationKeyPathKey
 */
typedef void (^BCKvoNotificationBlock)(id _Nullable observer, id object, NSDictionary<NSKeyValueChangeKey, id> *change);

/**
 @abstract BCKvoController makes Key-Value Observing simpler and safer.
 @discussion BCKvoController adds support for handling key-value changes with blocks and custom actions, as well as the NSKeyValueObserving callback. Notification will never message a deallocated observer. Observer removal never throws exceptions, and observers are removed implicitly on controller deallocation. BCKvoController is also thread safe. When used in a concurrent environment, it protects observers from possible resurrection and avoids ensuing crash. By default, the controller maintains a strong reference to objects observed.
 */
@interface BCKvoController : NSObject
