...
void CEFileEventRemoveFromReadTimer(CEPool_s * _Nonnull eventLoop, CEFileEvent_s * _Nonnull fileEvent);
void CEFileEventRemoveFromWriteTimer(CEPool_s * _Nonnull eventLoop, CEFileEvent_s * _Nonnull fileEvent);
void CEFileEventAddToReadTimer(CEPool_s * _Nonnull eventLoop, int feid, uint32_t timerId);
void CEFileEventAddToWriteTimer(CEPool_s * _Nonnull eventLoop, int feid, uint32_t timerId);
void CEFileEventUpdateReadTimer(CEPool_s * _Nonnull eventLoop, int feid, uint32_t timerId);
void CEFileEventUpdateWriteTimer(CEPool_s * _Nonnull eventLoop, int feid, uint32_t timerId);



CEFileEvent_s * _Nullable CEPoolGetFileEventByIndex(CEPool_s * _Nonnull eventLoop, int index) {
...
CEFileEvent_s * _Nullable CEPoolGetFileEvent(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd) {
    if (fd.fd >= eventLoop->setsize) {
        return NULL;
    }
    CEFileEvent_s *fe = CEPoolGetFileEventByIndex(eventLoop, fd.fd);
    if (NULL == fe) {
        return NULL;
    }
    if (NULL == fe->handler) {
...
static inline uint32_t CEFileEventTimeout(CEPool_s * _Nonnull eventLoop, uint32_t interval) {
...
//TODO: check
static inline uint32_t CEFileEventTimeId(CEPool_s * _Nonnull eventLoop, uint32_t interval) {
    return (eventLoop->timerFiredIndex + interval) & 0x1fff;
}

CEResult_t CEFileEventSourceSetReadTimeout(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd, uint32_t initialTimeout, uint32_t timeout) {
...
CEResult_t CEFileEventSourceSetWriteTimeout(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd, uint32_t initialTimeout, uint32_t timeout) {
    CEFileEvent_s * fe = CEPoolGetFileEvent(eventLoop, fd);
    if (NULL == fe) {
        return CEResultErrorFileDescription;
    }
...
CEResult_t CEFileEventSourceCancelReadTimeout(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd) {
    CEFileEvent_s * fe = CEPoolGetFileEvent(eventLoop, fd);
    if (NULL == fe) {
        return CEResultErrorFileDescription;
    }
...
CEResult_t CEFileEventSourceCancelWriteTimeout(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd) {
    CEFileEvent_s * fe = CEPoolGetFileEvent(eventLoop, fd);
    if (NULL == fe) {
        return CEResultErrorFileDescription;
    }
...
CEResult_t CEFileEventSourceUpdateReadTime(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd) {
    CEFileEvent_s * fe = CEPoolGetFileEvent(eventLoop, fd);
    if (NULL == fe) {
        return CEResultErrorFileDescription;
    }
...
CEResult_t CEFileEventSourceUpdateWriteTime(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd) {
    CEFileEvent_s * fe = CEPoolGetFileEvent(eventLoop, fd);
    if (NULL == fe) {
        return CEResultErrorFileDescription;
    }
...
CEFileEvent_s * _Nullable CEFileEventSourceInit(CEPool_s * _Nonnull eventLoop, CEFileDescription_s fd, CEFileEventHandler_f _Nonnull handler, void * _Nullable clientData) {
...
CEResult_t CEFileEventSourceDeinit(CEPool_s * _Nonnull eventLoop, CEFileEvent_s * _Nonnull fe) {
...
void CEFileEventRemoveFromReadTimer(CEPool_s * _Nonnull eventLoop, CEFileEvent_s * _Nonnull fileEvent) {
    ...
    if (prev < 0) {
        ...
        if (CEFileEventTimerListIsHead(prev, &index)) {
            CEFileEvent_s * nextFe = CEPoolGetFileEventByIndex(eventLoop, next);
            if (nextFe) {
        ...
        CEFileEvent_s * prevFe = CEPoolGetFileEventByIndex(eventLoop, prev);
        if (NULL == prevFe) {
...
void CEFileEventAddToReadTimer(CEPool_s * _Nonnull eventLoop, int feid, uint32_t timerId) {
    CEFileEvent_s * fe = CEPoolGetFileEventByIndex(eventLoop, feid);
    ...
    CEFileEvent_s * oldFe = CEPoolGetFileEventByIndex(eventLoop, old);
    if (oldFe) {
