...
static void CEApiPollDoTimer(void * _Nonnull api, void * _Nullable context, const CEApiPoolCallback_s * _Nonnull callback) {
    uint8_t buffer[1024] = {};
    recv(api->timerFd, buffer, 1024, MSG_DONTWAIT);
    callback->timerCallback(context, api);
}
...
int CEApiPoll(void * _Nonnull api, struct timeval * _Nullable tvp, int setsize, void * _Nullable context, const CEApiPoolCallback_s * _Nonnull callback) {
    ...
    if (retval > 0) {
        ...
        for (int j = 0; j < numevents; j++) {
            ...
            } else if (e->ident == state->timerFd) {
                CEApiPollDoTimer(state, context, callback);
            } else {
...
CEApiResult_t CEApiUpdateTimer(void * _Nonnull api, uint64_t when) {
    ...
    if (state->nextTimerTime == when) {
        return CEApiResultSuccess;
    }
    
    uint64_t now = CESystemBootInterval();
    uint64_t microseconds = 1;
    if (when > now) {
        microseconds = when - now;
    }
    state->nextTimerTime = when;
    
    struct itimerspec time = {};
    time.it_value.tv_sec = microseconds / 1000000ull;
    time.it_value.tv_nsec = (microseconds % 1000000ull) * 1000 + 500;
    if (timerfd_settime(fd, TFD_TIMER_ABSTIME, &time, NULL) != 0) {
        return CEApiResultErrorSystemCall;
    }
    return CEApiResultSuccess;
}
...
static void CEApiPollDoTimer(CEApiState_s * _Nonnull api, void * _Nullable context, const CEApiPoolCallback_s * _Nonnull callback) {
    CEApiState_s * state = api;
    state->timerIdent = (uintptr_t)(NULL);

    callback->timerCallback(context, state);
}
...
int CEApiPoll(void * _Nonnull api, struct timeval * _Nullable tvp, int setsize, void * _Nullable context, const CEApiPoolCallback_s * _Nonnull callback) {
    ...
    if (retval > 0) {
        ...
        for(int j = 0; j < numevents; j++) {
            ...
            if (e->ident == state->timerIdent) {
                CEApiPollDoTimer(state, context, callback);
            } else if (e->ident == state->pread) {
                CEApiPollDoPipe(state, context, callback);
            } else {
...
CEApiResult_t CEApiUpdateTimer(void * _Nonnull api, uint64_t when) {
    ...
    struct kevent ke;
    if (state->timerIdent) {
        EV_SET(&ke, state->timerIdent, EVFILT_TIMER, EV_DELETE, NOTE_USECONDS, microseconds, NULL);
        if (kevent(state->fd, &ke, 1, NULL, 0, NULL) == -1) {
            return CEApiResultErrorSystemCall;
        }
    }
    state->timerIdent = (uintptr_t)(&(state->timerIdent));
    EV_SET(&ke, state->timerIdent, EVFILT_TIMER, EV_ADD | EV_ONESHOT, NOTE_USECONDS, microseconds, NULL);
    if (kevent(state->fd, &ke, 1, NULL, 0, NULL) == -1) {
        return CEApiResultErrorSystemCall;
    }
...
CEApiResult_t CEApiRemoveTimer(void * _Nonnull api) {
    ...
    struct kevent ke;
    if (state->timerIdent) {
        EV_SET(&ke, state->timerIdent, EVFILT_TIMER, EV_DELETE, 0, 0, NULL);
        if (kevent(state->fd, &ke, 1, NULL, 0, NULL) == -1) {
            return CEApiResultErrorSystemCall;
        }
    }
