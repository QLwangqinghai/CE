...
NSString *const kBCKeychainErrorDomain = @"com.bichan.BCKeychain";

NSString *const kBCKeychainAccountKey = @"acct";
NSString *const kBCKeychainCreatedAtKey = @"cdat";
NSString *const kBCKeychainClassKey = @"labl";
NSString *const kBCKeychainDescriptionKey = @"desc";
NSString *const kBCKeychainLabelKey = @"labl";
NSString *const kBCKeychainLastModifiedKey = @"mdat";
NSString *const kBCKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef BCKeychainAccessibilityType = NULL;
#endif

@interface BCKeychainUtil ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation BCKeychainUtil
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = BCKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kBCKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (BCKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return BCKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (BCKeychainAccessibilityType) {
		CFRelease(BCKeychainAccessibilityType);
	}
	BCKeychainAccessibilityType = accessibilityType;
}
#endif
